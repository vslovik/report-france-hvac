...
class QuotePipeline:
    ...
    def __init__(self, df_clean: pd.DataFrame, start_date='2024-03-31', end_date=None):
        self.df_quotes = df_clean
        self.df_quotes['product'] = df_clean['regroup_famille_equipement_produit_principal'].map(Cohorts.map_product)
    ...
    def plot(self):
        ...
        for row, (thresh_label, days) in enumerate(self.THRESHOLDS.items()):
            for col, qtype in enumerate(['Single-quoter', 'Multi-quoter']):
                ...
                for snap_date in snapshots:
                    snap_quotes = self.df_quotes[self.df_quotes['dt_creation_devis'] <= snap_date].copy()
        ...
        plt.tight_layout()
        plt.savefig(f'{self.OUTPUT_DIR}/pipeline_threshold_comparison.png', dpi=150, bbox_inches='tight')
        plt.show()
